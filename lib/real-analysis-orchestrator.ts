...
export class RealAnalysisOrchestrator {
  ...
  private static async processDocumentAsync(
    ...
    try {
      ...
      const agentAnalyses = await this.runMultiAgentAnalysis(processedDoc, relevantKnowledge, reviewMode)
      
      // Step 6: Generate comprehensive analysis
      const finalAnalysis = await this.generateFinalAnalysis(processedDoc, agentAnalyses, relevantKnowledge)
